...
main() {
  ...
  if [ "$QUIET" -eq 0 ]; then
    ...
    echo ""
    echo "  Set your API key (pick one provider):"
    echo "    pi auth set anthropic  sk-ant-..."
    echo "    pi auth set openrouter sk-or-..."
    echo "    pi auth set openai     sk-..."
    echo ""
    echo "  Set default provider (optional — defaults to anthropic):"
    echo "    pi config set provider openrouter"
    echo ""
